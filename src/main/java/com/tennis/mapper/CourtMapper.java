...
public class CourtMapper implements DataMapper<Court>{
    ...
    public List<Court> findAvailableForReservations(Connection connection) throws SQLException{
        List<Court> courts = new ArrayList<>();
        String sql = "SELECT * FROM courts WHERE available_for_reservations = true".formatted();
        Statement statement = connection.createStatement();
        ResultSet rs = statement.executeQuery(sql);
    ...
    public List<Court> findBySurface(SurfaceType surfaceType, Connection connection) throws SQLException{
        List<Court> courts = new ArrayList<>();
        String sql = "SELECT * FROM courts WHERE surface_type = ?".formatted();
        PreparedStatement statement = connection.prepareStatement(sql);
        statement.setString(1, surfaceType.name());
        ResultSet rs = statement.executeQuery();
    ...
    public List<Court> findByRoof(Boolean hasRoof, Connection connection) throws SQLException{
        List<Court> courts = new ArrayList<>();
        String sql = "SELECT * FROM courts WHERE has_roof = ?".formatted();
        PreparedStatement statement = connection.prepareStatement(sql);
        statement.setBoolean(1,hasRoof);
        ResultSet rs = statement.executeQuery();
